...
opts = detectImportOptions('q2_data/bs_call_prices.csv');
opts = setvartype(opts, opts.VariableNames(2:end) ,'double');
bs_call_prices = readtable('q2_data/bs_call_prices.csv', opts);

% Load in the TTM values
opts = detectImportOptions('q2_data/ttm_call_prices.csv');
opts = setvartype(opts, opts.VariableNames(2:end) ,'double');
ttm_call_prices = readtable('q2_data/ttm_call_prices.csv', opts);

% Load in the S/X values
opts = detectImportOptions('q2_data/s_div_x.csv');
opts = setvartype(opts, opts.VariableNames(2:end) ,'double');
s_div_x = readtable('q2_data/s_div_x.csv', opts);

% Load in the K values
opts = detectImportOptions('q2_data/k_df.csv');
opts = setvartype(opts, opts.VariableNames(2:end) ,'double');
k = readtable('q2_data/k_df.csv', opts);
...
Y = [];
K = [];
for i = 1:length(bs_call_prices.Properties.VariableNames)
...
k_train = K(idx(1:round(P*m)),:) ; 
k_test = K(idx(round(P*m)+1:end),:) ;
...
C_X = Y_test ./ k_test;
scatter3(X_test(:,5),X_test(:,6),Y_test, '.');
...
% % Plot figure 5c
% figure();
% scatter3(X_test(:,5),X_test(:,6),Y_test-testing_labels, '.');
% colormap winter;
% hold on;
% ylabel('T-t');
% xlabel('S/K');
